...
describe("conta e experiência de carregamento do dashboard", () => {
  ...
  it("permite editar o nome e salvar preferências sem expor alteração de email", () => {
    ...
    expect(routerSource).toContain("z.string().trim().min(2");
    ...
    expect(layoutSource).toContain("O email é gerenciado pelo provedor de autenticação");
  ...
  it("renderiza skeletons animados e acessíveis durante a busca do panorama", () => {
    ...
    expect(overviewSource).toContain("Os indicadores serão exibidos quando os registros reais terminarem de carregar.");
