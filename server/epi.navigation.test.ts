...
const operationsPage = readFileSync(
  resolve(process.cwd(), "client/src/pages/Operations.tsx"),
  "utf8"
);
...
describe("navegação do Controle de EPIs", () => {
  ...
  it("mantém o atalho de EPI no dashboard Empresa e o contexto da rota", () => {
    expect(workspaceOverview).toContain(
      'title: "Controle de EPIs", text: "Estoque, CA, fichas de entrega e validade"'
    );
  ...
  it("permite abrir diretamente a aba de fichas e recupera separadamente falhas de acesso e de carregamento", async () => {
    ...
    expect(operationsPage).toContain(
      "Este ambiente não está disponível para a sua conta."
    );
    expect(operationsPage).toContain(
      "Não foi possível carregar os dados de EPIs agora."
    );
    expect(operationsPage).toContain(
      "Promise.all([workspace.refetch(), organization.refetch(), operations.refetch()])"
    );
  ...
  it("aplica filtros de setor e função e mantém a ficha completa fechada até abrir a gaveta", () => {
    ...
    expect(operationsPage).toContain(
      "employee.departmentId !== profileDepartmentFilter"
    );
    expect(operationsPage).toContain(
      "employee.jobRoleId !== profileRoleFilter"
    );
    expect(operationsPage).toContain(
      "expandedArchiveEmployeeId > 0 && profileEmployees.filter(emp => emp.id === expandedArchiveEmployeeId)"
    );
    expect(operationsPage).toContain(
  ...
  it("oferece fechamento explícito, persiste filtros por empresa e sinaliza a abertura da gaveta", () => {
    ...
    expect(operationsPage).toContain(
      "tst-hub:epi-profile-filters:${workspaceId}:${companyId}"
    );
    expect(operationsPage).toContain(
      "window.localStorage.getItem(archiveFiltersKey)"
    );
    expect(operationsPage).toContain(
      "window.localStorage.setItem(archiveFiltersKey, JSON.stringify(filters))"
    );
    expect(operationsPage).toContain(
      "openingArchiveEmployeeId === employee.id"
    );
  ...
  it("restaura o cadastro de estoque, a criação de ficha e a exportação com dados persistidos", () => {
    ...
    expect(operationsPage).toContain(
      "epiById.get(delivery.epiItemId)?.caNumber"
    );
  });
  ...
  it("recolhe a pasta e a ficha aberta, evita botão duplicado e persiste o aceite", () => {
    ...
    expect(operationsPage).toContain("Registrar ciência presencial");
  });
