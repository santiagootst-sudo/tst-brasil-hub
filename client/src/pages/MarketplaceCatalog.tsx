...
import { ArrowUpRight, BookOpen, CircleDollarSign, FileSpreadsheet, Filter, GraduationCap, PackageOpen, Search, ShoppingBag, SlidersHorizontal, Star, Tags, X } from "lucide-react";
...
export default function MarketplaceCatalog() {
  ...
  const [format, setFormat] = useState<"todos" | keyof typeof formatLabels>("todos");
  const [platform, setPlatform] = useState<"todos" | keyof typeof platformLabels>("todos");
  const [featuredOnly, setFeaturedOnly] = useState(false);
  const [sort, setSort] = useState<"featured" | "price-asc" | "price-desc">("featured");
  const trackCheckout = trpc.content.trackCheckout.useMutation();
  ...
  const formats = useMemo(() => ["todos", ...Array.from(new Set(materials.map(item => item.format)))], [materials]) as Array<"todos" | keyof typeof formatLabels>;
  const platforms = useMemo(() => ["todos", ...Array.from(new Set(materials.map(item => item.salePlatform)))], [materials]) as Array<"todos" | keyof typeof platformLabels>;
  const filteredMaterials = useMemo(() => {
    const term = search.trim().toLocaleLowerCase("pt-BR");
    return materials.filter(item => {
      const matchesCategory = category === "Todos" || item.category === category;
      const matchesFormat = format === "todos" || item.format === format;
      const matchesPlatform = platform === "todos" || item.salePlatform === platform;
      const content = `${item.title} ${item.description} ${item.category} ${item.format}`.toLocaleLowerCase("pt-BR");
      return matchesCategory && matchesFormat && matchesPlatform && (!featuredOnly || item.featured) && (!term || content.includes(term));
    }).sort((a, b) => {
      if (sort === "price-asc") return (a.priceCents ?? Number.MAX_SAFE_INTEGER) - (b.priceCents ?? Number.MAX_SAFE_INTEGER);
  ...
  }, [category, featuredOnly, format, materials, platform, search, sort]);
  const hasActiveFilters = category !== "Todos" || format !== "todos" || platform !== "todos" || featuredOnly || sort !== "featured" || Boolean(search);
  const clearFilters = () => { setSearch(""); setCategory("Todos"); setFormat("todos"); setPlatform("todos"); setFeaturedOnly(false); setSort("featured"); };
      ...
      <div className="mx-auto max-w-7xl space-y-7">
        ...
        <section className="rounded-[1.5rem] border border-[#dcebe8] bg-white p-4 shadow-sm sm:p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            ...
            {hasActiveFilters && <Button type="button" variant="outline" onClick={clearFilters} className="h-11 shrink-0 rounded-xl border-[#d3e5e0] text-xs font-bold text-[#397069]"><X className="mr-1.5 h-3.5 w-3.5" />Limpar</Button>}
          </div>
          <div className="mt-4 space-y-3 border-t border-[#edf4f1] pt-4"><div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-[.12em] text-[#668087]"><Tags className="h-3.5 w-3.5" /> Categorias</span>{categories.map(item => <button key={item} type="button" onClick={() => setCategory(item)} className={`rounded-lg px-3 py-2 text-xs font-bold transition ${category === item ? "bg-[#0c7474] text-white" : "bg-[#edf7f3] text-[#397069] hover:bg-[#d9eee7]"}`}>{item}</button>)}</div><div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-[.12em] text-[#668087]"><Filter className="h-3.5 w-3.5" /> Formato</span>{formats.map(item => <button key={item} type="button" onClick={() => setFormat(item)} className={`rounded-lg px-3 py-2 text-xs font-bold transition ${format === item ? "bg-[#e0f2ec] text-[#075d60] ring-1 ring-[#65af9e]" : "bg-[#f7fbfa] text-[#668087] hover:bg-[#edf7f3]"}`}>{item === "todos" ? "Todos" : formatLabels[item]}</button>)}</div><div className="flex flex-wrap items-center gap-2"><span className="text-[11px] font-bold uppercase tracking-[.12em] text-[#668087]">Compra</span>{platforms.map(item => <button key={item} type="button" onClick={() => setPlatform(item)} className={`rounded-lg px-3 py-2 text-xs font-bold transition ${platform === item ? "bg-[#e0f2ec] text-[#075d60] ring-1 ring-[#65af9e]" : "bg-[#f7fbfa] text-[#668087] hover:bg-[#edf7f3]"}`}>{item === "todos" ? "Todas" : platformLabels[item]}</button>)}<button type="button" onClick={() => setFeaturedOnly(current => !current)} className={`ml-1 inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs font-bold transition ${featuredOnly ? "bg-[#fff0c9] text-[#95640a] ring-1 ring-[#e5be5e]" : "bg-[#f7fbfa] text-[#668087] hover:bg-[#edf7f3]"}`}><Star className="h-3.5 w-3.5" />Destaques</button><label className="ml-auto flex items-center gap-2 text-xs font-bold text-[#668087]">Ordenar<select value={sort} onChange={event => setSort(event.target.value as typeof sort)} className="h-9 rounded-lg border border-[#d7e8e3] bg-white px-2 text-xs font-semibold text-[#315158]"><option value="featured">Destaques primeiro</option><option value="price-asc">Menor preço</option><option value="price-desc">Maior preço</option></select></label></div></div>
          <p className="mt-4 text-xs font-medium text-[#668087]">{filteredMaterials.length} {filteredMaterials.length === 1 ? "material encontrado" : "materiais encontrados"}</p>
        </section>
