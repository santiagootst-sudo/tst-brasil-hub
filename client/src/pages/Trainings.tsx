...
export default function Trainings() {
  ...
  return <DashboardLayout title="Treinamentos"><div className="mx-auto max-w-6xl pb-10">
    <section className="overflow-hidden rounded-[1.75rem] bg-[#063b43] p-7 text-white shadow-sm lg:p-9">
      <div className="grid gap-6 lg:grid-cols-[1.35fr_.65fr]"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#8edec7]">Gestão de capacitação</p><h2 className="mt-3 text-3xl font-bold tracking-tight">Organize o curso, a presença e a ata em um único registro.</h2><p className="mt-4 max-w-2xl text-sm leading-6 text-[#c5e2dc]">Registre instrutor, local, todas as datas e os participantes vinculados ao ambiente. A ata impressa organiza a lista de presença com linhas prontas para assinatura.</p></div><div className="rounded-3xl bg-white/8 p-5"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#8edec7]">Ambiente ativo</p><p className="mt-2 text-xl font-bold">{activeWorkspace.name}</p><p className="mt-1 text-xs text-[#c5e2dc]">{activeWorkspace.kind === "autonomo" ? "Prestador de serviço" : "Empresa"}</p></div></div>
      <div className="mt-7 flex flex-col justify-between gap-4 border-t border-white/10 pt-5 md:flex-row md:items-center"><div className="flex flex-wrap gap-2">{workspaces.data?.map(workspace => <button type="button" key={workspace.id} onClick={() => setLocation(`/app/treinamentos?workspace=${workspace.id}`)} className={`rounded-xl border px-3 py-2 text-sm font-semibold ${workspace.id === activeWorkspace.id ? "border-[#8edec7] bg-[#8edec7]/15 text-white" : "border-white/15 text-[#c5e2dc]"}`}>{workspace.name}</button>)}</div>{canManage && <Button onClick={() => setFormOpen(value => !value)} className="rounded-xl bg-[#8edec7] text-[#063b43] hover:bg-[#b0ebd8]"><Plus className="mr-2 h-4 w-4" />Planejar treinamento</Button>}</div>
    </section>
