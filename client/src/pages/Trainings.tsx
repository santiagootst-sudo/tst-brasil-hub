...
export default function Trainings() {
  ...
  const workspaceDetail = trpc.portal.workspace.useQuery({ workspaceId }, { enabled: Boolean(activeWorkspace) && workspaceId > 0 });
  ...
  const registeredCompany = workspaceDetail.data?.companies[0] ?? null;
  ...
  return <DashboardLayout title="Treinamentos"><ModulePage className="pb-10">
    ...
    {formOpen && <form onSubmit={event => { event.preventDefault(); const dates = scheduledDates.filter(Boolean).map(value => new Date(`${value}T12:00:00`)); createTraining.mutate({ workspaceId: activeWorkspace.id, companyId: registeredCompany?.id, title, instructorName: instructorName.trim() || null, location: locationName.trim() || null, scheduledAt: dates[0] ?? null, scheduledDates: dates, participantIds, participantCount: selectedParticipantCount }); }} className="mt-6 rounded-3xl border border-[#b9e3d7] bg-white p-6 shadow-sm">
