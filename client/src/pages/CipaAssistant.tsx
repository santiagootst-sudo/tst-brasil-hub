...
function Field({ label, hint, children, className = "" }: { label: string; hint?: string; children: React.ReactNode; className?: string }) {
  return (
    <label className={`block text-sm font-semibold text-[#315158] ${className}`}>
      <span className="flex items-center gap-2">{label}{hint && <span className="text-[11px] font-normal text-[#83a09a]">{hint}</span>}</span>
...
function downloadPdf(document: CipaDocument, companyName: string) {
  ...
  pdf.text(`Empresa: ${companyName || "não informada"} · Rascunho para revisão técnica`, margin, cursor);
  cursor += 10;
...
export default function CipaAssistant() {
  ...
  const [selectedDocument, setSelectedDocument] = useState<CipaDocument | null>(null);
  ...
  const generateDocuments = async () => {
    ...
    setGeneratedDocuments(buildCipaDocuments(form));
    setIsGenerating(false);
    setActiveStep(4);
    toast.success("Pacote de documentos preparado para revisão.");
  };
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-5 lg:grid-cols-[210px_1fr] lg:items-start">
          ...
          <main className="min-w-0 space-y-5">
            {activeStep === 0 && <div className="space-y-5"><SectionCard icon={ShieldCheck} eyebrow="Etapa 01 · identificação" title="Dados da empresa e do processo"><div className="grid gap-4 md:grid-cols-2"><Field label="Nome da empresa *" className="md:col-span-2"><Input value={form.empresa} onChange={event => update("empresa", event.target.value)} placeholder="Razão social da empresa" className="h-11 rounded-xl border-[#cfe3de]" /></Field><Field label="CNPJ *"><Input value={form.cnpj} onChange={event => update("cnpj", event.target.value)} placeholder="00.000.000/0000-00" className="h-11 rounded-xl border-[#cfe3de]" /></Field><Field label="Grau de risco *" hint="1 a 4"><select value={form.grauRisco} onChange={event => update("grauRisco", Number(event.target.value))} className="h-11 w-full rounded-xl border border-[#cfe3de] bg-white px-3 text-sm font-medium text-[#315158] outline-none"><option value={1}>Grau 1 · baixo</option><option value={2}>Grau 2 · moderado</option><option value={3}>Grau 3 · elevado</option><option value={4}>Grau 4 · muito elevado</option></select></Field><Field label="Número de empregados *"><Input type="number" min={0} value={form.empregados || ""} onChange={event => update("empregados", Number(event.target.value) || 0)} placeholder="Ex.: 85" className="h-11 rounded-xl border-[#cfe3de]" /></Field><Field label="Endereço completo" className="md:col-span-2"><Input value={form.endereco} onChange={event => update("endereco", event.target.value)} placeholder="Rua, número, bairro, cidade/UF" className="h-11 rounded-xl border-[#cfe3de]" /></Field><Field label="Sindicato da categoria" hint="se aplicável"><Input value={form.sindicato} onChange={event => update("sindicato", event.target.value)} placeholder="Nome do sindicato" className="h-11 rounded-xl border-[#cfe3de]" /></Field><Field label="Cidade/UF para assinatura *"><Input value={form.cidade} onChange={event => update("cidade", event.target.value)} placeholder="São Paulo/SP" className="h-11 rounded-xl border-[#cfe3de]" /></Field></div></SectionCard><div className="flex justify-end"><Button type="button" onClick={() => setActiveStep(1)} className="rounded-xl bg-[#0c7474] text-white hover:bg-[#063b43]">Continuar para dimensionamento <ArrowRight className="ml-2 h-4 w-4" /></Button></div></div>}
