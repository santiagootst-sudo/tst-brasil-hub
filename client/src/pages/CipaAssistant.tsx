...
export type CipaPdfDocument = { type: DocumentType; title: string; content: string; companyLogoUrl: string | null };
export type CipaPdfContext = {
  termLabel: string;
  city: string | null;
...
export async function buildCipaPdf(document: CipaPdfDocument, company: { name: string; document: string | null }, context?: CipaPdfContext) {
...
async function downloadCipaPdf(document: CipaPdfDocument, company: { name: string; document: string | null }, context?: CipaPdfContext) {
  const pdf = await buildCipaPdf(document, company, context);
  pdf.save(`cipa-${safeFileName(company.name)}-${safeFileName(document.title)}.pdf`);
}

export function documentContent(type: DocumentType, data: {
...
export default function CipaAssistant() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "election" | "meetings" | "members" | "documents">("overview");
  ...
  const [meetingDraft, setMeetingDraft] = useState({ title: "Reunião ordinária da CIPA", meetingType: "ordinary" as "ordinary" | "extraordinary", date: "", time: "09:00", location: "", agenda: "", minutesSummary: "", status: "scheduled" as "scheduled" | "completed" | "cancelled" });
  ...
  const meetings = selectedTerm ? snapshot.data?.meetings.filter(meeting => meeting.termId === selectedTerm.id).sort((a, b) => a.scheduledAt.getTime() - b.scheduledAt.getTime()) ?? [] : [];
  ...
  const createMeeting = trpc.portal.createCipaMeeting.useMutation({ onSuccess: async () => { await utils.portal.cipaSnapshot.invalidate(); setMeetingDraft({ title: "Reunião ordinária da CIPA", meetingType: "ordinary", date: "", time: "09:00", location: "", agenda: "", minutesSummary: "", status: "scheduled" }); toast.success("Reunião adicionada ao calendário CIPA."); }, onError: error => toast.error(error.message) });
  const updateMeeting = trpc.portal.updateCipaMeeting.useMutation({ onSuccess: async () => { await utils.portal.cipaSnapshot.invalidate(); setEditingMeetingId(null); setMeetingDraft({ title: "Reunião ordinária da CIPA", meetingType: "ordinary", date: "", time: "09:00", location: "", agenda: "", minutesSummary: "", status: "scheduled" }); toast.success("Reunião CIPA atualizada."); }, onError: error => toast.error(error.message) });
  ...
  const saveMeeting = () => {
    if (!activeWorkspace || !selectedCommission || !selectedTerm) return toast.error("Selecione uma gestão CIPA antes de registrar a reunião.");
    if (!meetingDraft.date || !meetingDraft.time) return toast.error("Informe a data e o horário da reunião.");
    const scheduledAt = new Date(`${meetingDraft.date}T${meetingDraft.time}:00`);
    if (Number.isNaN(scheduledAt.getTime())) return toast.error("A data ou o horário da reunião não é válido.");
    const input = { workspaceId: activeWorkspace.id, title: meetingDraft.title, meetingType: meetingDraft.meetingType, scheduledAt, location: meetingDraft.location.trim() || selectedCommission.workplace || null, agenda: meetingDraft.agenda.trim() || null, minutesSummary: meetingDraft.minutesSummary.trim() || null, status: meetingDraft.status };
    if (editingMeetingId) updateMeeting.mutate({ ...input, meetingId: editingMeetingId });
    else createMeeting.mutate({ ...input, commissionId: selectedCommission.id, termId: selectedTerm.id });
  };

  const editMeeting = (meeting: NonNullable<typeof snapshot.data>["meetings"][number]) => {
    const scheduled = new Date(meeting.scheduledAt);
    const local = new Date(scheduled.getTime() - scheduled.getTimezoneOffset() * 60_000);
    setEditingMeetingId(meeting.id);
    setMeetingDraft({ title: meeting.title, meetingType: meeting.meetingType, date: local.toISOString().slice(0, 10), time: local.toISOString().slice(11, 16), location: meeting.location || "", agenda: meeting.agenda || "", minutesSummary: meeting.minutesSummary || "", status: meeting.status });
  ...
  return <DashboardLayout title="CIPA"><div className="mx-auto max-w-7xl space-y-6">
    ...
    {!selectedCompany ? <section className="rounded-[1.75rem] border border-dashed border-[#c9ded8] bg-white p-10 text-center"><Building2 className="mx-auto h-10 w-10 text-[#0c7474]" /><h2 className="mt-4 text-xl font-bold text-[#102b32]">Cadastre a empresa antes de iniciar.</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[#668087]">A CIPA é sempre vinculada à empresa e seus documentos usam o logotipo cadastrado no respectivo ambiente.</p><Button onClick={() => setLocation(`/app/estrutura?workspace=${activeWorkspace.id}`)} variant="outline" className="mt-5 rounded-xl border-[#b9dcd2] text-[#0c7474]">Abrir Estrutura da empresa <ChevronRight className="ml-2 h-4 w-4" /></Button></section> : !selectedCommission || !selectedTerm ? <section className="rounded-[1.75rem] border border-dashed border-[#c9ded8] bg-white p-10 text-center"><ShieldCheck className="mx-auto h-11 w-11 text-[#0c7474]" /><h2 className="mt-4 text-xl font-bold text-[#102b32]">Nenhuma gestão CIPA para {selectedCompany.name}.</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[#668087]">Crie a gestão para registrar o cronograma eleitoral, os integrantes e os documentos desta empresa.</p><Button onClick={() => { setShowCreate(true); resetCompanyForm(String(selectedCompany.id)); }} className="mt-5 rounded-xl bg-[#0c7474] text-white"><Plus className="mr-2 h-4 w-4" />Criar gestão</Button></section> : <>
      ...
      <nav className="flex overflow-x-auto rounded-2xl border border-[#dcebe8] bg-white p-1.5 shadow-sm" aria-label="Navegação da gestão CIPA">{([ ["overview", "Visão da gestão", ShieldCheck], ["election", "Processo eleitoral", Vote], ["meetings", "Reuniões", CalendarDays], ["members", "Composição", UsersRound], ["documents", "Documentos", FileText] ] as const).map(([key, label, Icon]) => <button key={key} onClick={() => setActiveTab(key)} className={`flex min-w-max flex-1 items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold transition ${activeTab === key ? "bg-[#e8f6f1] text-[#0c7474] shadow-sm" : "text-[#668087] hover:bg-[#f6fbf9]"}`}><Icon className="h-4 w-4" />{label}</button>)}</nav>
      ...
      {activeTab === "meetings" && <div className="grid gap-5 xl:grid-cols-[1.08fr_.92fr]"><section className="rounded-[1.75rem] border border-[#dcebe8] bg-white p-5 shadow-sm"><SectionTitle icon={CalendarDays} eyebrow="Calendário persistente" title="Reuniões da gestão" description="As datas ficam vinculadas à empresa e ao mandato selecionado, mesmo após recarregar a página." /><div className="mt-5 space-y-3">{meetings.length ? meetings.map(meeting => <article key={meeting.id} className={`rounded-2xl border p-4 ${meeting.status === "cancelled" ? "border-[#f0d9d1] bg-[#fff9f7]" : meeting.status === "completed" ? "border-[#cde7da] bg-[#f5fbf8]" : "border-[#dcebe8] bg-white"}`}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex min-w-0 gap-3"><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${meeting.status === "completed" ? "bg-[#dbf2e5] text-[#18734d]" : meeting.status === "cancelled" ? "bg-[#ffe9e3] text-[#b64d2a]" : "bg-[#e5f4ef] text-[#0c7474]"}`}>{meeting.status === "completed" ? <CheckCircle2 className="h-5 w-5" /> : <CalendarDays className="h-5 w-5" />}</span><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-bold text-[#17383e]">{meeting.title}</h3><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${meeting.status === "completed" ? "bg-[#dff3e7] text-[#176d4b]" : meeting.status === "cancelled" ? "bg-[#fff0eb] text-[#b64926]" : "bg-[#e8f5f1] text-[#0b736f]"}`}>{meeting.status === "completed" ? "Realizada" : meeting.status === "cancelled" ? "Cancelada" : "Agendada"}</span></div><p className="mt-1 text-xs font-semibold text-[#52716f]">{new Date(meeting.scheduledAt).toLocaleString("pt-BR", { dateStyle: "long", timeStyle: "short" })} · {meeting.meetingType === "ordinary" ? "Ordinária" : "Extraordinária"}</p><p className="mt-1 text-xs text-[#78928d]">{meeting.location || "Local a definir"}</p>{meeting.agenda && <p className="mt-3 text-xs leading-5 text-[#5d7777]">Pauta: {meeting.agenda}</p>}</div></div><Button type="button" size="sm" variant="outline" onClick={() => editMeeting(meeting)} className="shrink-0 rounded-xl border-[#cfe4de] text-[#0c7474]">Editar</Button></div></article>) : <div className="rounded-2xl border border-dashed border-[#c7dfd8] bg-[#f8fcfa] px-5 py-10 text-center"><CalendarDays className="mx-auto h-9 w-9 text-[#0c7474]" /><p className="mt-3 font-bold text-[#315158]">Nenhuma reunião registrada neste mandato.</p><p className="mx-auto mt-1 max-w-md text-sm leading-6 text-[#668087]">Cadastre as reuniões ordinárias ou extraordinárias para manter o acompanhamento organizado.</p></div>}</div></section><section className="rounded-[1.75rem] border border-[#dcebe8] bg-white p-5 shadow-sm"><SectionTitle icon={editingMeetingId ? CheckCircle2 : Plus} eyebrow={editingMeetingId ? "Atualizar reunião" : "Nova reunião"} title={editingMeetingId ? "Edite a programação" : "Adicionar ao calendário"} description="Registre data, horário, pauta, local e o resultado da reunião." /><div className="mt-5 space-y-4"><label className="block text-xs font-bold text-[#315158]">Título<Input value={meetingDraft.title} onChange={event => setMeetingDraft(current => ({ ...current, title: event.target.value }))} className="mt-1.5 h-11 rounded-xl border-[#dcebe8]" placeholder="Ex.: Reunião ordinária de setembro" /></label><div className="grid grid-cols-2 gap-3"><label className="text-xs font-bold text-[#315158]">Tipo<select value={meetingDraft.meetingType} onChange={event => setMeetingDraft(current => ({ ...current, meetingType: event.target.value as typeof current.meetingType }))} className="mt-1.5 h-11 w-full rounded-xl border border-[#dcebe8] bg-white px-3 text-sm"><option value="ordinary">Ordinária</option><option value="extraordinary">Extraordinária</option></select></label><label className="text-xs font-bold text-[#315158]">Situação<select value={meetingDraft.status} onChange={event => setMeetingDraft(current => ({ ...current, status: event.target.value as typeof current.status }))} className="mt-1.5 h-11 w-full rounded-xl border border-[#dcebe8] bg-white px-3 text-sm"><option value="scheduled">Agendada</option><option value="completed">Realizada</option><option value="cancelled">Cancelada</option></select></label></div><div className="grid grid-cols-2 gap-3"><label className="text-xs font-bold text-[#315158]">Data<Input type="date" value={meetingDraft.date} onChange={event => setMeetingDraft(current => ({ ...current, date: event.target.value }))} className="mt-1.5 h-11 rounded-xl border-[#dcebe8]" /></label><label className="text-xs font-bold text-[#315158]">Horário<Input type="time" value={meetingDraft.time} onChange={event => setMeetingDraft(current => ({ ...current, time: event.target.value }))} className="mt-1.5 h-11 rounded-xl border-[#dcebe8]" /></label></div><label className="block text-xs font-bold text-[#315158]">Local<Input value={meetingDraft.location} onChange={event => setMeetingDraft(current => ({ ...current, location: event.target.value }))} className="mt-1.5 h-11 rounded-xl border-[#dcebe8]" placeholder={selectedCommission.workplace || "Sala, auditório ou unidade"} /></label><label className="block text-xs font-bold text-[#315158]">Pauta<Textarea value={meetingDraft.agenda} onChange={event => setMeetingDraft(current => ({ ...current, agenda: event.target.value }))} rows={3} className="mt-1.5 rounded-xl border-[#dcebe8]" placeholder="Itens previstos para a reunião" /></label><label className="block text-xs font-bold text-[#315158]">Resumo / ata<Textarea value={meetingDraft.minutesSummary} onChange={event => setMeetingDraft(current => ({ ...current, minutesSummary: event.target.value }))} rows={3} className="mt-1.5 rounded-xl border-[#dcebe8]" placeholder="Decisões e encaminhamentos, após a realização" /></label><div className="flex flex-wrap justify-end gap-2"><Button type="button" variant="outline" onClick={() => { setEditingMeetingId(null); setMeetingDraft({ title: "Reunião ordinária da CIPA", meetingType: "ordinary", date: "", time: "09:00", location: "", agenda: "", minutesSummary: "", status: "scheduled" }); }} className="rounded-xl border-[#cfe4de] text-[#51706e]">Limpar</Button><Button type="button" disabled={createMeeting.isPending || updateMeeting.isPending} onClick={saveMeeting} className="rounded-xl bg-[#0c7474] text-white">{createMeeting.isPending || updateMeeting.isPending ? "Salvando..." : editingMeetingId ? "Salvar reunião" : "Adicionar reunião"}</Button></div></div></section></div>}
