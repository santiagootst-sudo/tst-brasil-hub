...
type DashboardView = "resumo" | "cipa" | "epis" | "inspecoes" | "riscos" | "documentos";
...
export default function WorkspaceOverview() {
  ...
  const dashboardTabs: { id: DashboardView; label: string; description: string; icon: typeof ShieldCheck }[] = [
    ...
    ...(!isAutonomo ? [{ id: "riscos" as const, label: "Riscos", description: "PGR, controles e evolução", icon: ShieldCheck }] : []),
  ...
  return <DashboardLayout title="Dashboard"><div className="dashboard-readable mx-auto max-w-[1440px] space-y-4">
    <section className="sticky top-3 z-20 rounded-xl border border-[#dfe6ec] bg-white p-2 shadow-[0_4px_14px_rgba(15,42,68,.06)]" aria-label="Painéis do ambiente">
      <div className="flex flex-col gap-2">
        <div className="flex gap-2 overflow-x-auto px-1 pb-1 pt-1.5">
          {dashboardTabs.map(({ id, label, description, icon: Icon }) => { const badge = badgeCountFor(id); return <button key={id} type="button" onClick={() => setActiveDashboard(id)} aria-pressed={activeDashboard === id} className={`group relative flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 pr-7 text-left transition ${activeDashboard === id ? "bg-[#123f69] text-white shadow-sm" : "border border-[#e1e7ed] bg-white text-[#40556a] hover:bg-[#f5f8fb]"}`}><span className={`grid h-8 w-8 place-items-center rounded-lg ${activeDashboard === id ? "bg-white/15" : "bg-[#e8f6f1] text-[#0c7474]"}`}><Icon className="h-4 w-4" /></span><span><strong className="block text-xs font-bold">{label}</strong><small className="sr-only">{description}</small></span>{badge.count > 0 && <span className="absolute right-2 top-2 z-10 grid min-h-5 min-w-5 place-items-center rounded-full border-2 border-white bg-[#d83c3c] px-1 text-[9px] font-extrabold leading-none text-white shadow-sm" aria-label={`${badge.count} alerta(s) pendente(s) em ${label}`}>{badge.count > 99 ? "99+" : badge.count}</span>}</button>; })}
        </div>
        <div className="flex flex-col gap-2 rounded-lg border border-[#e1e7ed] bg-[#fbfcfd] px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
    ...
    <section className="relative overflow-hidden rounded-xl border border-[#dfe6ec] bg-white p-5 shadow-[0_4px_14px_rgba(15,42,68,.06)] lg:p-6">
      ...
      <div className="relative flex flex-col justify-between gap-7 lg:flex-row lg:items-start"><div><span className={`inline-flex rounded-full border border-white/80 bg-white/75 px-3 py-1 text-[10px] font-bold uppercase tracking-[.14em] shadow-sm ${context.accent}`}>{activeDashboard === "resumo" ? context.label : "Painel operacional"}</span><p className={`mt-5 text-xs font-bold uppercase tracking-[.14em] ${context.accent}`}>{activeDashboard === "resumo" ? context.eyebrow : `TST CLT · ${current.name}`}</p><h2 className="mt-2 max-w-3xl text-2xl font-bold tracking-[-.03em] text-[#173b43] lg:text-3xl">{activeDashboard === "resumo" ? context.headline : dashboardTabs.find(tab => tab.id === activeDashboard)?.label}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#5d7479]">{activeDashboard === "resumo" ? context.description : `${dashboardTabs.find(tab => tab.id === activeDashboard)?.description}. Consulte os indicadores deste recorte sem misturar informações.`}</p></div><Link href="/app" className="inline-flex items-center gap-2 rounded-xl border border-white/80 bg-white/45 px-4 py-2 text-sm font-bold text-[#0c7474] shadow-sm backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-white/75">Alternar contexto <ArrowRight className="h-4 w-4" /></Link></div>
      {activeDashboard === "resumo" && <div className="relative mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-7">{context.stats.map(({ label, value, icon: Icon, tone }) => <div key={label} className="rounded-xl border border-[#dfe6ec] bg-white p-4 shadow-[0_1px_3px_rgba(15,42,68,.05)] transition hover:-translate-y-0.5 hover:border-[#b8cbe0] hover:shadow-sm"><span className={`grid h-9 w-9 place-items-center rounded-xl ${tone === "coral" ? "bg-[#fff0e8] text-[#cf754a]" : tone === "blue" ? "bg-[#edf5fb] text-[#3173a8]" : "bg-[#e8f6f1] text-[#0c7474]"}`}><Icon className="h-4 w-4" /></span><p className="mt-4 text-3xl font-semibold tracking-tight text-[#173b43]">{value}</p><p className="mt-1 text-xs text-[#668087]">{label}</p></div>)}</div>}
    </section>
    ...
    {false && !hiddenSummaryWidgets.includes("hero") && <div style={{ order: summaryOrder.indexOf("hero") }}><section className="relative overflow-hidden rounded-xl border border-[#dfe6ec] bg-white p-5 shadow-[0_4px_14px_rgba(15,42,68,.06)] lg:p-6">
      <div className="pointer-events-none absolute -right-20 -top-24 h-72 w-72 rounded-full bg-[#8edec7]/20 blur-3xl" />
      <div className="pointer-events-none absolute bottom-[-8rem] left-1/3 h-64 w-64 rounded-full bg-[#b9defc]/15 blur-3xl" />
      <div className="relative flex flex-col justify-between gap-7 lg:flex-row lg:items-start"><div><span className={`inline-flex rounded-full border border-white/80 bg-white/75 px-3 py-1 text-[10px] font-bold uppercase tracking-[.14em] shadow-sm ${context.accent}`}>{context.label}</span><p className={`mt-5 text-xs font-bold uppercase tracking-[.14em] ${context.accent}`}>{context.eyebrow}</p><h2 className="mt-2 max-w-3xl text-2xl font-bold tracking-[-.03em] text-[#173b43] lg:text-3xl">{context.headline}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#5d7479]">{context.description}</p></div><Link href="/app" className="inline-flex items-center gap-2 rounded-xl border border-white/80 bg-white/45 px-4 py-2 text-sm font-bold text-[#0c7474] shadow-sm backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-white/75">Alternar contexto <ArrowRight className="h-4 w-4" /></Link></div>
      <div className="relative mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-7">{context.stats.map(({ label, value, icon: Icon, tone }) => <div key={label} className="rounded-xl border border-[#dfe6ec] bg-white p-4 shadow-[0_1px_3px_rgba(15,42,68,.05)] transition hover:-translate-y-0.5 hover:border-[#b8cbe0] hover:shadow-sm"><span className={`grid h-9 w-9 place-items-center rounded-xl ${tone === "coral" ? "bg-[#fff0e8] text-[#cf754a]" : tone === "blue" ? "bg-[#edf5fb] text-[#3173a8]" : "bg-[#e8f6f1] text-[#0c7474]"}`}><Icon className="h-4 w-4" /></span><p className="mt-4 text-3xl font-semibold tracking-tight text-[#173b43]">{value}</p><p className="mt-1 text-xs text-[#668087]">{label}</p></div>)}</div>
    </section></div>}
