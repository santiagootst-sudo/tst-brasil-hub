...
export default function WorkspaceOverview() {
  ...
  if (workspace.isLoading || certificates.isLoading || trainings.isLoading || organization.isLoading) {
  ...
  return <DashboardLayout title="Visão geral"><div className="mx-auto max-w-7xl space-y-7">
    <section className={`overflow-hidden rounded-[2rem] p-7 text-white shadow-lg lg:p-9 ${context.color}`}>
      ...
      <div className="mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{context.stats.map(({ label, value, icon: Icon, tone }) => <div key={label} className="rounded-2xl border border-white/10 bg-white/8 p-4"><span className={`grid h-9 w-9 place-items-center rounded-xl ${tone === "coral" ? "bg-[#e98766]/15 text-[#ffb69d]" : tone === "blue" ? "bg-[#b9defc]/15 text-[#b9defc]" : "bg-[#8edec7]/15 text-[#8edec7]"}`}><Icon className="h-4 w-4" /></span><p className="mt-4 text-3xl font-bold">{value}</p><p className="mt-1 text-xs text-white/70">{label}</p></div>)}</div>
    </section>
