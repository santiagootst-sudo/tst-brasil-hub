...
export default function WorkspaceOverview() {
  ...
  return <DashboardLayout title="Visão geral"><div className="mx-auto max-w-7xl space-y-7">
    <section className={`overflow-hidden rounded-[2rem] border p-7 shadow-sm lg:p-9 ${context.color}`}>
      <div className="flex flex-col justify-between gap-7 lg:flex-row lg:items-start"><div><span className={`inline-flex rounded-full border border-white/80 bg-white/75 px-3 py-1 text-[10px] font-bold uppercase tracking-[.14em] shadow-sm ${context.accent}`}>{context.label}</span><p className={`mt-5 text-xs font-bold uppercase tracking-[.14em] ${context.accent}`}>{context.eyebrow}</p><h2 className="mt-2 max-w-3xl text-3xl font-semibold tracking-[-.03em] text-[#173b43] lg:text-4xl">{context.headline}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#5d7479]">{context.description}</p></div><Link href="/app" className="inline-flex items-center gap-2 rounded-xl border border-[#b9d8d1] bg-white/75 px-4 py-2 text-sm font-bold text-[#0c7474] shadow-sm transition hover:bg-white">Alternar contexto <ArrowRight className="h-4 w-4" /></Link></div>
      <div className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-7">{context.stats.map(({ label, value, icon: Icon, tone }) => <div key={label} className="rounded-2xl border border-[#d7e8e3] bg-white/80 p-4 shadow-[0_8px_24px_rgba(28,74,77,0.06)]"><span className={`grid h-9 w-9 place-items-center rounded-xl ${tone === "coral" ? "bg-[#fff0e8] text-[#cf754a]" : tone === "blue" ? "bg-[#edf5fb] text-[#3173a8]" : "bg-[#e8f6f1] text-[#0c7474]"}`}><Icon className="h-4 w-4" /></span><p className="mt-4 text-3xl font-semibold tracking-tight text-[#173b43]">{value}</p><p className="mt-1 text-xs text-[#668087]">{label}</p></div>)}</div>
    </section>
