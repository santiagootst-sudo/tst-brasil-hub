...
export default function WorkspaceHub() {
  ...
  const { data: workspaces, isLoading } = trpc.portal.workspaces.useQuery(undefined, {
  ...
  const createWorkspace = trpc.portal.createWorkspace.useMutation({
    onSuccess: workspace => {
      ...
      toast.success("Ambiente criado. O painel deste contexto já está pronto para uso.");
    },
  ...
  return (
    <DashboardLayout title="Início">
      <div className="mx-auto max-w-6xl">
        {billingSuccess && (
          <div className="mb-5 rounded-2xl border border-[#b9e3d7] bg-[#f1fcf7] px-5 py-4 text-sm text-[#17664f]">
            Recebemos a confirmação do checkout. A assinatura será liberada assim que o pagamento for processado.
          </div>
        )}
          ...
          <div className="mt-7 grid gap-6 lg:grid-cols-2">
            {(["autonomo", "clt"] as WorkspaceKind[]).map(kind => {
              ...
              return (
                <article
                  key={kind}
                  className={`overflow-hidden rounded-[1.8rem] border bg-white shadow-sm ${autonomous ? "border-[#a6ddcf]" : "border-[#c5dff3]"}`}
                >
                  <div className={`flex min-h-56 items-start justify-between p-7 ${autonomous ? "bg-[#f4fcf8]" : "bg-[#f4f9ff]"}`}>
                    <div>
                      <span className={`rounded-lg px-3 py-1 text-[10px] font-bold uppercase tracking-[.12em] ${autonomous ? "bg-[#d9f1e7] text-[#168c89]" : "bg-[#dbeeff] text-[#2165a9]"}`}>
                        TST {contextLabel}
                      </span>
                      <h4 className="mt-5 text-2xl font-bold">
                        {autonomous ? "Atue como prestador de serviços" : "Atue como colaborador da empresa"}
                      </h4>
                      <p className="mt-3 max-w-sm text-sm leading-6 text-[#5d7479]">
                        {autonomous
                          ? "Priorize clientes, empresas, entregas e rotinas de consultoria."
                          : "Priorize pessoas, indicadores, conformidade e a rotina interna de SST."}
                      </p>
                    </div>
                    <span className={`grid h-14 w-14 place-items-center rounded-2xl ${autonomous ? "bg-[#d9f1e7] text-[#0c8c89]" : "bg-[#dbeeff] text-[#2165a9]"}`}>
      ...
      {form && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-[#062f35]/45 p-5">
          <form
            onSubmit={event => {
          ...
          >
            ...
            <label className="mt-6 block text-sm font-semibold">
              Nome do ambiente
              <Input value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} className="mt-2 h-11 rounded-xl" autoFocus />
            </label>
            <div className="mt-6 flex justify-end gap-3">
              <Button type="button" variant="ghost" onClick={() => setForm(null)}>Cancelar</Button>
              <Button disabled={createWorkspace.isPending || form.name.trim().length < 2} className="rounded-xl bg-[#0c7474] text-white">
                {createWorkspace.isPending ? "Criando" : "Criar e abrir painel"}
              </Button>
            </div>
          </form>
        </div>
