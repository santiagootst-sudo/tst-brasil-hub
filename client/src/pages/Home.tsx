...
const pillars = [
  { icon: FileCheck2, title: "PGR Pro como primeiro aplicativo", text: "Estruture inventários, riscos, medidas de controle e documentos com um fluxo desenhado para o TST." },
  { icon: UsersRound, title: "Dois contextos, uma conta", text: "Atue como prestador de serviços ou profissional CLT sem misturar dados, rotinas e empresas." },
  { icon: BookOpen, title: "Ecossistema que cresce com você", text: "Biblioteca, materiais, treinamentos, certificados e marketplace em uma mesma experiência." },
];
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#f7fbfa] text-[#102b32]">
      ...
      <section className="relative mx-auto grid max-w-7xl gap-12 px-6 pb-20 pt-16 lg:grid-cols-[1.06fr_.94fr] lg:px-8 lg:pb-28 lg:pt-24">
        ...
        <div className="relative">
          ...
          <h1 className="mt-6 max-w-3xl font-display text-5xl font-extrabold leading-[1.04] tracking-[-.055em] text-[#102b32] sm:text-6xl">A plataforma que une o <span className="text-[#0c8c89]">PGR</span> ao dia a dia do TST.</h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-[#49636a]">Portal TST Brasil é a base de trabalho para o Técnico de Segurança do Trabalho atuar como autônomo, CLT — ou ambos — com dados isolados por ambiente e uma experiência única.</p>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row"><Button onClick={enter} size="lg" className="h-13 rounded-xl bg-[#0c7474] px-6 text-base text-white hover:bg-[#063b43]">Começar com o PGR Pro <ArrowRight className="ml-2 h-4 w-4" /></Button><Link href="/planos" className="inline-flex h-13 items-center justify-center rounded-xl border border-[#b7d8d2] bg-white px-6 text-sm font-bold text-[#0c7474] hover:border-[#0c7474]">Ver planos mensais</Link></div>
          <div className="mt-9 flex flex-wrap gap-x-6 gap-y-3 text-sm font-medium text-[#315158]"><span className="inline-flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[#39a77e]" /> Ambientes separados</span><span className="inline-flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[#39a77e]" /> Cobrança recorrente</span><span className="inline-flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[#39a77e]" /> PGR como app central</span></div>
        </div>
      ...
      <section id="produto" className="border-y border-[#deece9] bg-white py-20"><div className="mx-auto max-w-7xl px-6 lg:px-8"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#0c8c89]">Base do ecossistema</p><h2 className="mt-3 max-w-2xl text-4xl font-bold tracking-[-.04em]">Tudo começa por um PGR mais confiável e uma jornada mais clara.</h2><div className="mt-12 grid gap-5 lg:grid-cols-3">{pillars.map(({ icon: Icon, title, text }) => <article key={title} className="rounded-3xl border border-[#deece9] bg-[#fbfefd] p-7"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#e8f6f1] text-[#0c7474]"><Icon className="h-5 w-5" /></span><h3 className="mt-6 text-xl font-bold">{title}</h3><p className="mt-3 text-sm leading-6 text-[#5d7479]">{text}</p></article>)}</div></div></section>

      <section id="como-funciona" className="mx-auto max-w-7xl px-6 py-20 lg:px-8"><div className="rounded-[2rem] bg-[#102b32] px-8 py-12 text-white lg:px-14"><div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr]"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#9fd6ca]">Próximo passo</p><h2 className="mt-3 text-4xl font-bold tracking-[-.04em]">Sua operação SST não precisa caber em ferramentas desconectadas.</h2></div><div className="grid gap-5 sm:grid-cols-3">{["Crie a conta", "Selecione o ambiente", "Ative o PGR Pro"].map((step, index) => <div key={step}><span className="text-3xl font-bold text-[#88ddc4]">0{index + 1}</span><p className="mt-4 text-sm font-semibold">{step}</p><p className="mt-2 text-sm leading-6 text-[#b6d9d2]">A jornada mantém dados, permissões e cobrança organizados desde o início.</p></div>)}</div></div></div></section>
    </main>
  );
}
