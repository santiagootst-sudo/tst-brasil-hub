...
import { Copy, KeyRound, MessageCircle, PackagePlus, ShieldCheck, UserCheck, UserMinus, UserPlus, UserRound, UsersRound, Video } from "lucide-react";
...
export default function AdminPanel() {
  ...
  return <DashboardLayout title="Gestão de acessos"><div className="mx-auto max-w-7xl space-y-7">
    <section className="relative overflow-hidden rounded-[2rem] bg-[#073b43] px-6 py-8 text-white shadow-[0_24px_60px_rgba(7,59,67,.16)] lg:px-9"><div className="pointer-events-none absolute -right-20 -top-24 h-64 w-64 rounded-full bg-[#77cdb2]/20 blur-3xl" /><div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-2xl"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.18em] text-[#9be2cd]"><ShieldCheck className="h-4 w-4" />Administração do ecossistema</div><h2 className="mt-3 font-display text-3xl font-bold tracking-tight lg:text-4xl">Acessos sob controle, sem perder o contexto.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-[#c9e6df]">Crie, renove, bloqueie e acompanhe os acessos. Todas as credenciais são geradas no momento da liberação.</p></div><div className="flex flex-wrap items-center gap-3"><Link href="/admin/materiais" className="inline-flex h-11 items-center rounded-xl border border-white/15 bg-white/10 px-4 text-xs font-bold text-white transition hover:bg-white/20"><PackagePlus className="mr-2 h-4 w-4 text-[#9be2cd]" />Gerir materiais</Link><Link href="/admin/videos" className="inline-flex h-11 items-center rounded-xl border border-white/15 bg-white/10 px-4 text-xs font-bold text-white transition hover:bg-white/20"><Video className="mr-2 h-4 w-4 text-[#ffb5ad]" />Gerir vídeos</Link><div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-sm text-[#d9eeea]"><p className="font-semibold text-white">Administrador autenticado</p><p className="mt-1 text-xs">{user?.email || user?.name || "Proprietário do portal"}</p></div></div></div></section>
