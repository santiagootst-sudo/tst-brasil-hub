...
import { Copy, KeyRound, ShieldCheck, UserCheck, UserMinus, UserPlus, UserRound, UsersRound } from "lucide-react";
...
export default function AdminPanel() {
  ...
  const [latestCredential, setLatestCredential] = useState<{ name: string; email: string; password: string; expiresAt: Date | string } | null>(null);
  ...
  const grantAccess = trpc.admin.grantAccess.useMutation({
    onSuccess: async data => {
      setLatestCredential({ name: data.request.fullName, email: data.request.email, password: data.temporaryPassword, expiresAt: data.expiresAt });
  ...
  const createManualAccess = trpc.admin.createManualAccess.useMutation({
    onSuccess: async data => {
      setLatestCredential({ name: data.request.fullName, email: data.request.email, password: data.temporaryPassword, expiresAt: data.expiresAt });
      try { await navigator.clipboard.writeText(`E-mail: ${data.request.email}\nSenha: ${data.temporaryPassword}`); } catch { /* clipboard is optional */ }
      toast.success(`Acesso criado para ${data.request.email}.`);
      setNewAccess({ fullName: "", email: "", phone: "", companyName: "", jobTitle: "" });
      await Promise.all([utils.admin.accessRequests.invalidate(), utils.admin.users.invalidate(), utils.admin.audits.invalidate()]);
    },
    onError: error => toast.error(error.message),
  });
  const resetCredential = trpc.admin.resetCredential.useMutation({
    onSuccess: async data => {
      setLatestCredential({ name: data.request.fullName, email: data.request.email, password: data.temporaryPassword, expiresAt: data.expiresAt });
      try { await navigator.clipboard.writeText(`E-mail: ${data.request.email}\nSenha: ${data.temporaryPassword}`); } catch { /* clipboard is optional */ }
      toast.success(`Nova senha gerada para ${data.request.email}.`);
      await Promise.all([utils.admin.accessRequests.invalidate(), utils.admin.users.invalidate()]);
    },
    onError: error => toast.error(error.message),
  });
  ...
  return (
    <DashboardLayout title="Gestão de acessos">
      <div className="mx-auto max-w-7xl space-y-7">
        ...
        <Card className="overflow-hidden rounded-[1.75rem] border-[#b9ddd3] shadow-[0_14px_40px_rgba(16,43,50,.07)]"><CardHeader className="border-b border-[#e4f0ed] bg-[#f5fcf9] px-5 py-5 lg:px-7"><CardTitle className="flex items-center gap-2 font-display text-xl text-[#102b32]"><UserPlus className="h-5 w-5 text-[#0c8c89]" />Criar novo acesso</CardTitle><p className="mt-1 text-sm text-[#668087]">Cadastre a pessoa, escolha a validade e gere uma senha temporária para enviar pelo canal de atendimento.</p></CardHeader><CardContent className="p-5 lg:p-7"><form onSubmit={event => { event.preventDefault(); createManualAccess.mutate({ ...newAccess, durationDays }); }} className="grid gap-4 lg:grid-cols-3"><Input value={newAccess.fullName} onChange={event => setNewAccess(current => ({ ...current, fullName: event.target.value }))} required placeholder="Nome completo" className="h-11 rounded-xl border-[#dcebe8]" /><Input value={newAccess.email} onChange={event => setNewAccess(current => ({ ...current, email: event.target.value }))} required type="email" placeholder="E-mail de acesso" className="h-11 rounded-xl border-[#dcebe8]" /><Input value={newAccess.phone} onChange={event => setNewAccess(current => ({ ...current, phone: event.target.value }))} placeholder="Telefone / WhatsApp" className="h-11 rounded-xl border-[#dcebe8]" /><Input value={newAccess.companyName} onChange={event => setNewAccess(current => ({ ...current, companyName: event.target.value }))} placeholder="Empresa (opcional)" className="h-11 rounded-xl border-[#dcebe8]" /><Input value={newAccess.jobTitle} onChange={event => setNewAccess(current => ({ ...current, jobTitle: event.target.value }))} placeholder="Função (opcional)" className="h-11 rounded-xl border-[#dcebe8]" /><Button type="submit" disabled={createManualAccess.isPending} className="h-11 rounded-xl bg-[#0c7474] font-bold text-white hover:bg-[#063b43]">{createManualAccess.isPending ? "Gerando..." : "Gerar acesso"}<KeyRound className="ml-2 h-4 w-4" /></Button></form>{latestCredential && <div className="mt-5 flex flex-col gap-3 rounded-2xl border border-[#bfe2d4] bg-[#ebf8f1] p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.12em] text-[#18734d]">Credencial pronta para envio</p><p className="mt-1 text-sm font-bold text-[#102b32]">{latestCredential.name} · {latestCredential.email}</p><p className="mt-1 font-mono text-sm text-[#315158]">Senha: {latestCredential.password}</p><p className="mt-1 text-xs text-[#668087]">Válida até {formatDate(latestCredential.expiresAt)}</p></div><Button type="button" variant="outline" onClick={async () => { try { await navigator.clipboard.writeText(`E-mail: ${latestCredential.email}\nSenha: ${latestCredential.password}`); toast.success("Credenciais copiadas."); } catch { toast.error("Não foi possível copiar automaticamente."); } }} className="rounded-xl border-[#8fcbb7] text-[#0c7474]"><Copy className="mr-2 h-4 w-4" />Copiar</Button></div>}</CardContent></Card>
