...
export default function Organization() {
  ...
  const createEmployee = trpc.portal.createEmployee.useMutation({ onSuccess: async () => { setEmployeeName(""); setEmployeeDepartmentId(0); setEmployeeRoleId(0); await refresh(); toast.success("Pessoa registrada."); }, onError: error => toast.error(error.message) });
  ...
  return <DashboardLayout title="Estrutura e equipe"><div className="mx-auto max-w-7xl space-y-6">
    <section className={`overflow-hidden rounded-[2rem] p-7 text-white shadow-lg lg:p-9 ${current.kind === "clt" ? "bg-[#123f69]" : "bg-[#063b43]"}`}><div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#8edec7]">Base operacional</p><h2 className="mt-2 text-3xl font-bold">Estrutura, funções e pessoas.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-white/75">Cadastre a estrutura real de cada empresa. Esses registros serão a base para PGR, capacitações, certificados e indicadores operacionais.</p></div><div className="rounded-2xl border border-white/15 bg-white/10 p-4 text-sm"><p className="text-white/65">{current.kind === "autonomo" ? "Empresa atendida" : "Empresa da operação"}</p><p className="mt-1 font-bold">{currentCompany?.name ?? "Selecione uma empresa"}</p></div></div></section>
    ...
    {!companies.length ? <section className="rounded-3xl border border-dashed border-[#bddbd5] bg-[#fbfefd] p-10 text-center"><Building2 className="mx-auto h-10 w-10 text-[#0c7474]" /><h3 className="mt-4 text-xl font-bold">Cadastre uma empresa antes de estruturar a equipe.</h3><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[#668087]">Setores, funções e pessoas sempre pertencem a uma empresa e ao ambiente selecionado.</p><Link href={`/app/pgr?workspace=${current.id}`} className="mt-6 inline-flex rounded-xl bg-[#0c7474] px-5 py-3 text-sm font-bold text-white">Abrir empresas e PGR</Link></section> : <>
      ...
      <section className="grid gap-5 xl:grid-cols-3">
        ...
        <article className="rounded-3xl border border-[#dcebe8] bg-white p-5 shadow-sm"><div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#fff0e9] text-[#d67845]"><UsersRound className="h-5 w-5" /></span><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#bd6e4f]">Pessoas</p><h3 className="text-lg font-bold">Equipe</h3></div></div>{canManage && <div className="mt-5 space-y-3"><Input value={employeeName} onChange={event => setEmployeeName(event.target.value)} placeholder="Nome completo" /><select value={employeeDepartmentId} onChange={event => setEmployeeDepartmentId(Number(event.target.value))} className="h-10 w-full rounded-xl border border-[#cfe3de] bg-white px-3 text-sm"><option value={0}>Selecionar setor (opcional)</option>{departments.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={employeeRoleId} onChange={event => setEmployeeRoleId(Number(event.target.value))} className="h-10 w-full rounded-xl border border-[#cfe3de] bg-white px-3 text-sm"><option value={0}>Selecionar função (opcional)</option>{jobRoles.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><Button disabled={createEmployee.isPending || employeeName.trim().length < 2} onClick={() => createEmployee.mutate({ workspaceId, companyId: currentCompanyId, departmentId: employeeDepartmentId || null, jobRoleId: employeeRoleId || null, fullName: employeeName.trim(), hiredAt: null })} className="w-full rounded-xl bg-[#d67845] text-white"><UserRoundPlus className="mr-2 h-4 w-4" />Adicionar pessoa</Button></div>}<div className="mt-5 space-y-2">{employees.length ? employees.map(item => <div key={item.id} className="rounded-xl border border-[#e6f0ee] p-3"><strong className="block text-sm">{item.fullName}</strong><small className="mt-1 block text-xs text-[#668087]">{roleNameById.get(item.jobRoleId ?? 0) ?? "Função não informada"} · {departmentNameById.get(item.departmentId ?? 0) ?? "Setor não informado"}</small></div>) : <p className="rounded-xl bg-[#fffaf7] p-3 text-sm text-[#668087]">Nenhuma pessoa registrada para esta empresa.</p>}</div></article>
      </section>
    </>}
  </div></DashboardLayout>;
}
