  ...
  return <section className="space-y-5">
    <section className="relative overflow-hidden rounded-[2rem] border border-[#b9e3d7] bg-[linear-gradient(135deg,#effbf7_0%,#ffffff_55%,#eaf7f1_100%)] p-6 shadow-[0_18px_45px_rgba(28,74,77,.08)] lg:p-7">
      ...
      <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div><p className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.16em] text-[#0c8c89]"><Sparkles className="h-3.5 w-3.5" />PGR + inspeções</p><h3 className="mt-2 text-2xl font-bold tracking-[-.03em] text-[#102b32]">Painel de riscos ocupacionais</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-[#5d7479]">Veja rapidamente o que precisa de ação: riscos ativos, prioridades e situação de cada setor.</p></div>
      ...
      <div className="relative mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Riscos ativos" value={metrics.active} detail="Ainda exigem acompanhamento" tone="blue" /><MetricCard label="Alta prioridade" value={metrics.high} detail="Alto ou crítico" tone={metrics.high ? "coral" : "mint"} /><MetricCard label="Em tratamento" value={risks.filter(item => item.companyId === currentCompanyId && item.situation === "in_treatment").length} detail="Com ação de controle em andamento" tone="amber" /><MetricCard label="Eliminados" value={metrics.eliminated} detail="Perigo removido e registrado" tone="mint" /></div>
    </section>
