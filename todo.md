...
- [x] Implementar isolamento rigoroso de arquivos e dados por workspace e empresa no backend (prevenção de acesso cruzado).
- [x] Criar a separação na interface da Biblioteca entre "Normas Oficiais e Manuais Globais" e "Biblioteca Interna da Empresa".
...
- [x] Exportar o código do TST Brasil Hub para o GitHub (`santiagootst-sudo/tst-brasil-hub`).
- [x] Configurar o arquivo de automação `render.yaml` e documentar o processo de deploy e domínio personalizado (`tstbrasilhub.com.br`) no Render e Registro.br.
...
- [x] Cobrir o módulo CIPA com testes, revisar a interface, enviar ao GitHub e publicar no Render.
- [x] Corrigir a barra lateral móvel e tornar o sino de notificações acionável com o motivo de cada alerta.
...
- [x] Acionar no Render o deploy manual do commit d37e9be enviado ao GitHub.
- [x] Verificar que o portal publicado respondeu corretamente após a atualização.
...
- [x] Validar conteúdos, seleção de curso e geração de PDF das novas capacitações antes da publicação.

- [x] Sincronizar no GitHub o catálogo ampliado de cursos, conteúdos sugeridos e imagens temáticas aprovados.
- [x] Publicar no Render e confirmar a disponibilidade do gerador ampliado de certificados.

- [x] Remover o cabeçalho redundante “Documentos que sustentam a rotina de SST” da página de certificados.
